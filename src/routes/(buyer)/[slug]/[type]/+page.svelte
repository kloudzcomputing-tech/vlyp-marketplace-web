...
<div class="min-h-screen bg-gradient-to-b from-gray-50 to-white">
    <div class="flex flex-col lg:flex-row">
        ...
        <div class="lg:hidden sticky top-0 z-30 bg-white/95 backdrop-blur-sm border-b border-gray-200 shadow-sm">
            <div class="px-4 py-4 flex items-center justify-between">
                ...
                >
                    ...
                    <span>Filters</span>
                    {#if activeFilterCount > 0}
                        <span class="absolute -top-1 -right-1 w-5 h-5 bg-[var(--primary-color)] text-white text-xs font-bold rounded-full flex items-center justify-center">
                            {activeFilterCount}
                        </span>
                    {/if}
                </button>
        ...
        >
            <div class="p-5 sm:p-6 lg:p-8 space-y-6 lg:space-y-8 h-full overflow-y-auto custom-scrollbar">
                <!-- Mobile Header -->
                <div class="flex items-center justify-between lg:hidden pb-5 border-b border-gray-200">
                    <div>
                ...
                <div class="space-y-3">
                    <button
                        ...
                        onclick={() => accordianOpen.Categories = !accordianOpen.Categories}
                    >
                        <h3 class="text-sm font-semibold text-gray-900 uppercase tracking-wide">
                            Categories
                        </h3>
                        <ChevronDown
                            class="w-4 h-4 text-gray-500 transition-transform duration-200 {accordianOpen.Categories ? 'rotate-180' : ''}"
                        />
                    </button>
                ...
                <div class="space-y-3">
                    <button
                        type="button"
                        class="w-full flex items-center justify-between group"
                        onclick={() => accordianOpen.Discounts = !accordianOpen.Discounts}
                    >
                        <h3 class="text-sm font-semibold text-gray-900 uppercase tracking-wide">
                            Discounts
                        </h3>
                        <ChevronDown
                            class="w-4 h-4 text-gray-500 transition-transform duration-200 {accordianOpen.Discounts ? 'rotate-180' : ''}"
                        />
                    </button>
                    <div
                        ...
                        {#each discounts as discount}
                            ...
                            >
                                <input
                                    ...
                                    id={discount}
                                    name="Discounts"
                                    onchange={(e) => toggleFilter("Discounts", discount, e.target.checked)}
                                    checked={selectedFilters["Discounts"].includes(discount)}
                ...
                <div class="space-y-3">
                    <button
                        type="button"
                        class="w-full flex items-center justify-between group"
                        onclick={() => accordianOpen.Ratings = !accordianOpen.Ratings}
                    >
                        <h3 class="text-sm font-semibold text-gray-900 uppercase tracking-wide">
                            Ratings
                        </h3>
                        <ChevronDown
                            class="w-4 h-4 text-gray-500 transition-transform duration-200 {accordianOpen.Ratings ? 'rotate-180' : ''}"
                        />
                    </button>
                    <div
                        ...
                        {#each ratings as rating}
                            ...
                            >
                                <input
                                    ...
                                    id={rating.value}
                                    name="Ratings"
                                    onchange={(e) => toggleFilter("Ratings", rating.value, e.target.checked)}
                                    checked={selectedFilters["Ratings"].includes(rating.value)}
                                    class="w-4 h-4 rounded border-gray-300 text-[var(--primary-color)] focus:ring-none  cursor-pointer transition-all duration-200"
                                />
                                <span class="text-sm text-gray-700 group-hover:text-gray-900 transition-colors duration-200 font-medium flex-1 flex items-center gap-2">
                ...
                <div class="space-y-3">
                    <button
                        type="button"
                        class="w-full flex items-center justify-between group"
                        onclick={() => accordianOpen.Sellers = !accordianOpen.Sellers}
                    >
                        <h3 class="text-sm font-semibold text-gray-900 uppercase tracking-wide">
                            Sellers
                        </h3>
                        <ChevronDown
                            class="w-4 h-4 text-gray-500 transition-transform duration-200 {accordianOpen.Sellers ? 'rotate-180' : ''}"
                        />
                    </button>
                    <div
                        ...
                        {#each sellers as seller}
                            ...
                            >
                                <input
                                    ...
                                    id={seller.slug}
                                    name="Sellers"
                                    onchange={(e) => toggleFilter("Sellers", seller.slug, e.target.checked)}
                                    checked={selectedFilters["Sellers"].includes(seller.slug)}
                                    class="w-4 h-4 rounded border-gray-300 text-[var(--primary-color)] focus:ring-none  cursor-pointer transition-all duration-200"
                                />
                                <span class="text-sm text-gray-700 group-hover:text-gray-900 transition-colors duration-200 font-medium flex-1">
                                    {seller.display_name}
                ...
                {#each attributes as attribute}
                    <div class="space-y-3">
                        <button
                            type="button"
                            class="w-full flex items-center justify-between group"
                            onclick={() => accordianOpen[attribute.label] = !accordianOpen[attribute.label]}
                        >
                            <h3 class="text-sm font-semibold text-gray-900 uppercase tracking-wide">
                                {attribute.label}
                            </h3>
                            <ChevronDown
                                class="w-4 h-4 text-gray-500 transition-transform duration-200 {accordianOpen[attribute.label] ? 'rotate-180' : ''}"
                            />
                        </button>
                        <div
                            class="space-y-2  transition-all duration-300  {accordianOpen[attribute.label]
                                ? 'block'
                                : 'hidden'}"
                        >
                            {#each JSON.parse(attribute.values) as value}
                                ...
                                >
                                    <input
                                        ...
                                        onchange={(e) => toggleFilter(attribute.label, value, e.target.checked)}
                                        checked={selectedFilters[attribute.label].includes(value)}
                                        class="w-4 h-4 rounded border-gray-300 text-[var(--primary-color)] focus:ring-none  cursor-pointer transition-all duration-200"
                                    />
                                    <span class="text-sm text-gray-700 group-hover:text-gray-900 transition-colors duration-200 font-medium flex-1">
                                        {value}
