...
<div class="col-span-3 overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm transition-shadow duration-300 hover:shadow-md md:col-span-1">
  <!-- Header -->
    ...
    <div class="grid grid-cols-2 gap-3">
      {#each card.items as collection, index}
        <a
          ...
          class="card-item group cursor-pointer overflow-hidden rounded-lg border border-slate-200 bg-white transition-all duration-300 hover:border-slate-300 hover:shadow-md"
        ...
        >
          ...
          <div class="p-3 sm:p-4">
            {#if collection.title}
              <h3 class="line-clamp-2 text-sm font-semibold text-slate-900 transition-colors group-hover:text-[var(--primary-color)]">
                {collection.title}
              </h3>
            {/if}
            {#if collection.subtitle}
              <p class="line-clamp-2 pt-1 text-xs font-semibold text-emerald-700">
                {collection.subtitle}
              </p>
            {/if}
          </div>
